...
public class RestRecipe {
	...
	private final RecipeDTO recipeDTO = DTOManager.createRecipeDTO();
	...
	public @ResponseBody List<Recipe> get(@PathVariable("keyword") String keyword) {
		...
		if(recipes.isEmpty()){
			Integer page = 1;
			Integer resultPerPage = 100;
			recipes = searchRecipes(keyword, page, resultPerPage);
		}
		return recipes;
	}
	...
	@RequestMapping(value="/testrecipe")
	@ResponseBody
	public Recipe testrecipe(@RequestParam("keyword") String keyword, 
			@RequestParam(value="page", required=true) String page,
			@RequestParam(value="resultPerPage", required=true) String resultPerPage){
	...
	private Recipe fetchRecipe(int recipeId) {
		...
		String requestBody = sendRequest(uri);
		Gson gson = new Gson();
	...
	private List<Recipe> searchRecipes(String keyword, Integer page, Integer resultPerPage) {
		String requestBody = searchRecipeResult();
